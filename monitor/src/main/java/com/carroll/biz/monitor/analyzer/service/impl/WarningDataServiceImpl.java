...
import com.carroll.biz.monitor.analyzer.enums.StatisticsType;
...
import com.carroll.biz.monitor.common.dto.*;
...
public class WarningDataServiceImpl extends BaseServiceImpl implements IWarningDataService {
    ...
    public WarningData save(WarningData warningData) {
        ...
        if (WarningData.Status.CLEARED.equals(warningData.getStatus())) {
            ...
            warningDataRepository.delete(warningData);
        } else {
        ...
        if (count > 0) {
            ...
            for (int i = 1; i <= batch; i++) {
                ...
                list.forEach(data -> {
                    ...
                    MonitorItem monitorItem = monitorItemRepository.findById(data.getItemId()).orElse(null);
                    if (null != monitorItem) {
                ...
                list.forEach(data -> {
                    ...
                    if (null != monitorItem && monitorItem.getSendFlag()) {
                        ...
                        if (null != monitorItem.getCycle() && null != monitorItem.getCycleTimes() && null != data.getCycleTimes()) {
                            ...
                            int result = OldDateUtils.compare_date(now, nextSendTime);
                            if (result >= 0 && data.getCycleTimes() >= monitorItem.getCycleTimes()) {
                                NotifyDataDto notifyDataDto = new NotifyDataDto(false, monitorItem, data);
    ...
    public HistoryMonitorPageResponse historyMonitor(HistoryMonitorPageRequest request) {
        ...
        if (page != null) {
            ...
            page.getContent().forEach(data -> {
                MonitorItem item = monitorItemRepository.findById(data.getItemId()).orElse(null);
    ...
    public List<String> verifyPorjectAndGetMonitorItemIds(String projectId, String projectTag, String itemId, String level) throws BaseException {
     ...
     */
    @Override
    public MonitorDetailResponse monitorDetail(IdRequest request) throws BaseException {
        MonitorDetailResponse response = new MonitorDetailResponse();
        WarningData data = warningDataRepository.findById(request.getId()).orElse(null);
        WarningDataHistory history = warningDataHistoryRepository.findById(request.getId()).orElse(null);
        ...
        MonitorItem item = monitorItemRepository.findById(null == data ? history.getItemId() : data.getItemId()).orElse(null);
        if (null == item) {
        ...
        if (data == null) {
            ...
            dto.setTarget(parseTargetName(history.getTarget(), history.getProjectId()));
            ...
            dto.setTarget(parseTargetName(data.getTarget(), data.getProjectId()));
    ...
    private String parseTargetName(String code, String projectId) {
        if (!StringUtils.isEmpty(code)) {
            String targetName = monitorTargetService.getTargetName(code, projectId);
            return StringUtils.isEmpty(targetName) ? code : targetName;
        }
        return code;
    }
    ...
    public MonitorStatisticResponse statistic(MonitorStatisticRequest request) throws BaseException {
    ...
    public TraceDetailResponse getTraceDetail(String traceId) {
        ...
        try {
            OkHttpUtil.OkResponse okResponse = OkHttpUtil.doGet(String.format("%s?id=%s", traceUrl, traceId));
            response.setData(JSONObject.parseObject(okResponse.getResult()));
        } catch (Exception e) {
    ...
    private List<WarningDataStatisticDto> statisticByName(String projectId, String status) {
        ...
        list.forEach(data -> {
            WarningDataStatisticDto dto = new WarningDataStatisticDto();
            MonitorItem item =monitorItemRepository.findById(data.getItemId()).orElse(null);
            dto.setDesc(item==null?null:item.getName());
            dto.setTimes(data.getTimes());
            dto.setStatus(data.getStatus().getDesc());
