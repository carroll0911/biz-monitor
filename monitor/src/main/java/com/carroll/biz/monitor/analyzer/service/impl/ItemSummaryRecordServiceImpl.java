...
import com.carroll.biz.monitor.analyzer.repository.ItemSummaryRecordRepository;
...
public class ItemSummaryRecordServiceImpl implements IItemSummaryRecordService {
    ...
    public ItemSummaryRecord findFromRedis(String itemId, Date date) {
        String key = String.format("ISR_%s_%s", itemId, OldDateUtils.getStrDateFormat(date, OldDateUtils.YMD_DATE_FORMAT));
        return (ItemSummaryRecord) redisUtil.get(key);
    }

    @Override
    public ItemSummaryRecord save(ItemSummaryRecord record) {
        String key = String.format("ISR_%s_%s", record.getItemId(), OldDateUtils.getStrDateFormat(record.getDate(), OldDateUtils.YMD_DATE_FORMAT));
...
//        List<String> itemIds = Arrays.asList("5d5f908d37941db84fe554f0", "5d3540f320577f25c0555330", "5d3540f620577f25c0555332");
        Date end = OldDateUtils.dayBegin(new Date());
        Map<String, List<ItemSummaryRecord>> allDataMap = getSummaryData4Line(projectId, type, OldDateUtils.addInteger(end, Calendar.DATE, -15), end);
//        List<ItemSummaryRecord> records = itemSummaryRecordRepository.findAllByItemIdInAndDateBetween(itemIds, DateUtils.addInteger(end, Calendar.DATE, -15), end);
        ...
        for (int i = 15; i > 0; i--) {
            response.getDates().add(OldDateUtils.addInteger(end, Calendar.DATE, 0 - i));
        }
        ...
        for (Map.Entry<String, List<ItemSummaryRecord>> e : allDataMap.entrySet()) {
            monitorItem = monitorItemRepository.findById(e.getKey()).orElse(null);
            if (monitorItem == null) {
                continue;
            }
            response.getItemNames().add(monitorItem.getName());
            for (ItemSummaryRecord record : e.getValue()) {
                dataMap.put(String.format("%s#%s", monitorItem.getName(),
                        OldDateUtils.getStrDateFormat(record.getDate(), OldDateUtils.YMD_DATE_FORMAT)), record.getFailPer());
            }
        }
        ...
        for (String itemName : response.getItemNames()) {
            ...
            for (Date date : response.getDates()) {
                data = dataMap.get(String.format("%s#%s", itemName, OldDateUtils.getStrDateFormat(date, OldDateUtils.YMD_DATE_FORMAT)));
